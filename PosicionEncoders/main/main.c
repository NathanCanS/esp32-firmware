...
void encoder_init(void) {
    // Configuracion del control de pulsos
    pcnt_unit_config_t pcnt_config_r = {
        .high_limit = PCNT_H_LIM,
        .low_limit = PCNT_L_LIM,
    };
    
    ESP_ERROR_CHECK(pcnt_new_unit(&pcnt_config_r, &pcnt_unit_r));
    
    // Filtro para eliminar ruido
    pcnt_glitch_filter_config_t filter_config = {
        .max_glitch_ns = 1000,
    };
    ESP_ERROR_CHECK(pcnt_unit_set_glitch_filter(pcnt_unit_r, &filter_config));
    
    // Encoder derecho
    pcnt_chan_config_t chan_config_r = {
    .edge_gpio_num = ENC_R_A,
    .level_gpio_num = ENC_R_B,
	};
	pcnt_channel_handle_t pcnt_chan_r = NULL;
	ESP_ERROR_CHECK(pcnt_new_channel(pcnt_unit_r, &chan_config_r, &pcnt_chan_r));
	
	// Acciones del encoder r
	ESP_ERROR_CHECK(pcnt_channel_set_edge_action(pcnt_chan_r, 
        PCNT_CHANNEL_EDGE_ACTION_DECREASE, PCNT_CHANNEL_EDGE_ACTION_INCREASE));
    ESP_ERROR_CHECK(pcnt_channel_set_level_action(pcnt_chan_r, 
        PCNT_CHANNEL_LEVEL_ACTION_KEEP, PCNT_CHANNEL_LEVEL_ACTION_INVERSE));
    
    
    // Habilitar e iniciar encoder r
    ESP_ERROR_CHECK(pcnt_unit_enable(pcnt_unit_r));
    ESP_ERROR_CHECK(pcnt_unit_clear_count(pcnt_unit_r));
    ESP_ERROR_CHECK(pcnt_unit_start(pcnt_unit_r));
    
    // Encoder izquierdo
    
    ESP_ERROR_CHECK(pcnt_new_unit(&pcnt_config_r, &pcnt_unit_l));
    ESP_ERROR_CHECK(pcnt_unit_set_glitch_filter(pcnt_unit_l, &filter_config));
    
   pcnt_chan_config_t chan_config_l = {
        .edge_gpio_num = ENC_L_A,
        .level_gpio_num = ENC_L_B,
    };
    pcnt_channel_handle_t pcnt_chan_l = NULL;
    ESP_ERROR_CHECK(pcnt_new_channel(pcnt_unit_l, &chan_config_l, &pcnt_chan_l));
    
    // Configuración de acciones para encoder l
    ESP_ERROR_CHECK(pcnt_channel_set_edge_action(pcnt_chan_l, 
        PCNT_CHANNEL_EDGE_ACTION_DECREASE, PCNT_CHANNEL_EDGE_ACTION_INCREASE));
    ESP_ERROR_CHECK(pcnt_channel_set_level_action(pcnt_chan_l, 
        PCNT_CHANNEL_LEVEL_ACTION_KEEP, PCNT_CHANNEL_LEVEL_ACTION_INVERSE));
    
    // Habilitar e iniciar encoder l
    ESP_ERROR_CHECK(pcnt_unit_enable(pcnt_unit_l));
    ESP_ERROR_CHECK(pcnt_unit_clear_count(pcnt_unit_l));
    ESP_ERROR_CHECK(pcnt_unit_start(pcnt_unit_l));
}
...
void read_encoders(void) {
    ...
    int current_count_r = 0, current_count_l = 0;
    
    // Conteo de posicion actual
    ESP_ERROR_CHECK(pcnt_unit_get_count(pcnt_unit_r, &current_count_r));
    ESP_ERROR_CHECK(pcnt_unit_get_count(pcnt_unit_l, &current_count_l));
    
    //printf("R_raw: %d, L_raw: %d ",current_count_r,current_count_l);
    
    // Diferencia de posicion
    incr = current_count_r - last_count_r;
    incl = current_count_l - last_count_l;
    
    //printf("R_diff: %d, L_diff: %d ",incr,incl);
